...
export class PlaSpinner extends LitElement {
	static override styles = css`
		...
		.lds-roller div {
			animation: lds-roller var(--speed, 1.2s) cubic-bezier(0.5, 0, 0.5, 1) infinite;
			transform-origin: calc(0.5 * var(--size, 80px)) calc(0.5 * var(--size, 80px));
		}
		...
		.lds-roller div:after {
			...
			margin: calc(-0.05 * var(--size, 80px)) 0 0 calc(-0.05 * var(--size, 80px));
		}
